...
このスライドは、ROSを使ったロボットアームの制御方法について学ぶ教材です。MoveItフレームワーク、IKソルバー、JointTrajectoryController実装について詳しく解説します。約45枚のスライドで構成され、6つのインタラクティブなアプリケーションが含まれています。
...
2. **ロボット制御の書籍**
   ...
   - 左に画像、右に説明文
3. **このスライドの目的**
   - メーカーが用意した制御では難しいロボットアームの軌道制御をするための資料
4. **アームの軌道制御とは、位置制御との違い**
   - 左：軌道制御の例(Robotic_powder_grinding.gif)
   - 右：位置制御と軌道制御の比較説明

### 前提知識編（水平スライド5 + 縦スライド）
5. **アーム制御の前提知識**（メインスライド）
   - **縦スライド5-1**: ロボットアームの構造（左に画像、右に説明文）
   - **縦スライド5-2**: アーム制御は組合せ（制御方法×座標系×制御種類の表）
   - **縦スライド5-3**: 多くのロボットは位置/速度制御（画像配置：image2,5,8,12,15をバランス良く配置）
   - **縦スライド5-4**: よく使うROSのアームコントローラ（JointTrajectoryController、CartesianControllers）
   - **縦スライド5-5**: ROSのアームコントローラの命名規則（テキストのみ）
   - **縦スライド5-6**: 本編の内容（MoveIt、JointTrajectoryController）

### MoveItによるアーム制御編（水平スライド6 + 縦スライド）
6. **MoveItによるアーム制御** （メインスライド）
    - **縦スライド6-1**: MoveItで何ができるか
    - **縦スライド6-2**: MoveItのモーションプランニングの手順
    - **縦スライド6-3**: 手先の姿勢の表現（image13を30%サイズに縮小）
    - **縦スライド6-4**: 姿勢の表現 - オススメの実装
    - **縦スライド6-5**: 逆運動学の必要性
    - **縦スライド6-6**: 逆運動学の解き方（IKソルバー）
    - **縦スライド6-7**: MoveIt2で使えるIKソルバー
    - **縦スライド6-8**: Pythonで使えるIKソルバー
    - **縦スライド6-9**: MoveIt2で使えるパスプランニング
    - **縦スライド6-10**: サンプリングベースの手法（RRT）
    - **縦スライド6-11**: 最適化ベースの手法（CHOMP）
    - **縦スライド6-12**: 最適化ベースの手法（STOMP）

### JointTrajectoryControllerによるアーム制御編（水平スライド7 + 縦スライド）
7. **JointTrajectoryControllerによるアーム制御** （メインスライド）
    - **縦スライド7-1**: JTCでの実装の流れは5ステップ
    - **縦スライド7-2**: 目標姿勢の計算
    - **縦スライド7-3**: 軌跡・軌道生成
    - **縦スライド7-4**: 実行
    - **縦スライド7-5**: JTCの実装例
